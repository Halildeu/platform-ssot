...
backend_deploy_payload="$(build_json \
  ...
  VAULT_SECRET_PREFIX "${VAULT_SECRET_PREFIX:-${ENV_NAME}}" \
  VAULT_KV_MOUNT "${VAULT_KV_MOUNT:-secret}" \
  VAULT_FAIL_FAST "${VAULT_FAIL_FAST:-true}" \
  SPRING_CLOUD_VAULT_ENABLED "${SPRING_CLOUD_VAULT_ENABLED:-true}" \
  SPRING_CLOUD_VAULT_KV_ENABLED "${SPRING_CLOUD_VAULT_KV_ENABLED:-true}" \
