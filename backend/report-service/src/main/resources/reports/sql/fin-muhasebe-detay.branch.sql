...
OUTER APPLY (
  ...
  FROM (
    ...
    FROM (
      ...
      UNION ALL
      -- EXPENSE_ITEM_PLANS_MONEY has no ACTION_MONEY_ID column (live evidence
      -- 2026-05-05): composite key (ACTION_ID + MONEY_TYPE). NULL stand-in for
      -- audit log + ORDER BY tie-break (priority/is_selected dominate).
      SELECT 'EXPENSE_ITEM_PLANS_MONEY', ACTION_ID, MONEY_TYPE, RATE2, IS_SELECTED, CAST(NULL AS bigint) AS ACTION_MONEY_ID
      ...
      UNION ALL
      -- TAHAKKUK_PLAN_MONEY uses `ID` as the row identity (live evidence
      -- 2026-05-05); no ACTION_MONEY_ID column. Aliased so the union shape
      -- matches the rest.
      SELECT 'TAHAKKUK_PLAN_MONEY', ACTION_ID, MONEY_TYPE, RATE2, IS_SELECTED, ID AS ACTION_MONEY_ID
    ...
    FROM (
      SELECT 'INVOICE_MONEY' AS action_table, ACTION_ID, MONEY_TYPE, RATE2, IS_SELECTED, ACTION_MONEY_ID AS action_money_id
        FROM [{schema}].[INVOICE_MONEY] WITH (NOLOCK) WHERE MONEY_TYPE = 'EUR'
      UNION ALL
      -- See L3 above for column-shape rationale (EXPENSE_ITEM_PLANS_MONEY +
      -- TAHAKKUK_PLAN_MONEY have non-standard money-id columns; live
      -- evidence 2026-05-05).
      SELECT 'EXPENSE_ITEM_PLANS_MONEY', ACTION_ID, MONEY_TYPE, RATE2, IS_SELECTED, CAST(NULL AS bigint) AS ACTION_MONEY_ID
        FROM [{schema}].[EXPENSE_ITEM_PLANS_MONEY] WITH (NOLOCK) WHERE MONEY_TYPE = 'EUR'
      UNION ALL
