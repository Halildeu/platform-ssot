...
import type {
  TimezonePreferenceMutationRequest,
  TimezonePreferenceMutationResponse,
  TimezonePreferenceSnapshot,
} from "./timezonePreference.contract";

const DEFAULT_SOURCE = "mobile-offline-queue";
const DEFAULT_RETRY_AFTER_MS = 30_000;

function buildTimezonePreferenceUrl() {
  return `${resolveGatewayBaseUrl().replace(/\/+$/, "")}/v1/users/me/timezone`;
}
...
export async function fetchTimezonePreferenceSnapshot(
  token: string,
): Promise<TimezonePreferenceSnapshot> {
  const response = await fetch(buildTimezonePreferenceUrl(), {
    method: "GET",
    headers: buildAuthorizedHeaders(token),
  });

  const payload = await readJsonSafe<TimezonePreferenceSnapshot>(response);
  if (!response.ok || !payload) {
    throw new Error(getErrorMessage(payload, `Timezone snapshot failed: ${response.status}`));
  }

  return payload;
}

function mapQueuedActionToMutationRequest(action: QueuedAction): TimezonePreferenceMutationRequest {
  return {
    timezone: normalizeTimezone(action.targetTimezone ?? "Europe/Istanbul"),
...
function mapHttpFailureResult(response: Response, payload: unknown): OfflineQueueMutationResult {
  ...
  const message = getErrorMessage(payload, `Timezone mutation failed: ${response.status}`);
...
export function createTimezonePreferenceSyncMutationAdapter(
  token: string,
): OfflineQueueMutationAdapter {
  return {
    execute: async (action: QueuedAction): Promise<OfflineQueueMutationResult> => {
      if (action.kind !== "profile.timezone.sync") {
        return {
          ...
          message: "Timezone adapter cannot handle this queued action.",
        };
      }
      ...
      try {
        response = await fetch(buildTimezonePreferenceUrl(), {
          method: "PUT",
          headers: buildAuthorizedHeaders(token),
          body: JSON.stringify({
      ...
      const payload = await readJsonSafe<TimezonePreferenceMutationResponse>(response);
      if (response.status === 409 && payload) {
        return {
          ...
          message: payload.message || "Timezone sync conflict detected.",
        };
      }
