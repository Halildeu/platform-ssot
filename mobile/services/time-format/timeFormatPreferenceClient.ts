...
import type {
  TimeFormatPreferenceMutationRequest,
  TimeFormatPreferenceMutationResponse,
  TimeFormatPreferenceSnapshot,
} from "./timeFormatPreference.contract";

const DEFAULT_SOURCE = "mobile-offline-queue";
const DEFAULT_RETRY_AFTER_MS = 30_000;

function buildTimeFormatPreferenceUrl() {
  return `${resolveGatewayBaseUrl().replace(/\/+$/, "")}/v1/users/me/time-format`;
}
...
function normalizeTimeFormat(timeFormat: string) {
  return timeFormat.trim();
}

export async function fetchTimeFormatPreferenceSnapshot(
  token: string,
): Promise<TimeFormatPreferenceSnapshot> {
  const response = await fetch(buildTimeFormatPreferenceUrl(), {
    method: "GET",
    headers: buildAuthorizedHeaders(token),
  });

  const payload = await readJsonSafe<TimeFormatPreferenceSnapshot>(response);
  if (!response.ok || !payload) {
    throw new Error(getErrorMessage(payload, `Time-format snapshot failed: ${response.status}`));
  }

  return payload;
}

function mapQueuedActionToMutationRequest(action: QueuedAction): TimeFormatPreferenceMutationRequest {
  return {
    timeFormat: normalizeTimeFormat(action.targetTimeFormat ?? "HH:mm"),
...
function mapHttpFailureResult(response: Response, payload: unknown): OfflineQueueMutationResult {
  ...
  const message = getErrorMessage(payload, `Time-format mutation failed: ${response.status}`);
...
export function createTimeFormatPreferenceSyncMutationAdapter(
  token: string,
): OfflineQueueMutationAdapter {
  return {
    execute: async (action: QueuedAction): Promise<OfflineQueueMutationResult> => {
      if (action.kind !== "profile.time-format.sync") {
        return {
          ...
          message: "Time-format adapter cannot handle this queued action.",
        };
      }
      ...
      try {
        response = await fetch(buildTimeFormatPreferenceUrl(), {
          method: "PUT",
          headers: buildAuthorizedHeaders(token),
          body: JSON.stringify({
        ...
        const message =
          error instanceof Error && error.message.trim()
            ? error.message
            : "Time-format sync request failed.";
      ...
      const payload = await readJsonSafe<TimeFormatPreferenceMutationResponse>(response);
      if (response.status === 409 && payload) {
        return {
          ...
          message: payload.message || "Time-format sync conflict detected.",
        };
      }
