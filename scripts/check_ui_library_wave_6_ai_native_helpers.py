...
ROOT = Path(".")
CONTRACT = ROOT / "docs/02-architecture/context/ui-library-wave-6-ai-native-helpers.v1.json"
...
REQUIRED_GATE_COMMAND = "npm -C web run gate:ui-library-wave -- --wave wave_6_ai_native_helpers"
EXPECTED_COMPONENTS = {
    "CommandPalette",
    "RecommendationCard",
    "ConfidenceBadge",
...
def main() -> int:
    ...
    if missing:
        print("[check_ui_library_wave_6_ai_native_helpers] FAIL")
        for path in missing:
    ...
    if data.get("wave_id") != "wave_6_ai_native_helpers":
        problems.append("invalid-wave-id")
    if data.get("family_id") != "ai_native_helpers":
    ...
    for item in components:
        ...
        if not registry_item:
            ...
            if registry_item.get("roadmapWaveId") != "wave_6_ai_native_helpers":
                problems.append(f"registry-wave:{name}:{registry_item.get('roadmapWaveId')}")
            if registry_item.get("acceptanceContractId") != "ui-library-wave-6-ai-native-helpers-v1":
    ...
    print("[check_ui_library_wave_6_ai_native_helpers] OK components=%d" % len(components))
    return 0
