...
ROOT = Path('.')
CONTRACT = ROOT / 'docs/02-architecture/context/ui-library-wave-5-overlay.v1.json'
...
def main() -> int:
    ...
    if missing:
        print('[check_ui_library_wave_5_overlay] FAIL')
        for path in missing:
    ...
    if data.get('wave_id') != 'wave_5_overlay':
    ...
    for item in components:
        ...
        if not registry_item:
            ...
            if registry_item.get('roadmapWaveId') != 'wave_5_overlay':
                problems.append(f'registry-wave:{name}:{registry_item.get("roadmapWaveId")}')
            if registry_item.get('acceptanceContractId') != 'ui-library-wave-5-overlay-v1':
    ...
    print('[check_ui_library_wave_5_overlay] OK components=%d' % len(components))
    return 0
