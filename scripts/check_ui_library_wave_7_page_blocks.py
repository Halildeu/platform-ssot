...
ROOT = Path(".")
CONTRACT = ROOT / "docs/02-architecture/context/ui-library-wave-7-page-blocks.v1.json"
...
REQUIRED_GATE_COMMAND = "npm -C web run gate:ui-library-wave -- --wave wave_7_page_blocks"
EXPECTED_COMPONENTS = {
    "PageLayout",
    "FilterBar",
    "ReportFilterPanel",
...
def main() -> int:
    ...
    if missing:
        print("[check_ui_library_wave_7_page_blocks] FAIL")
        for path in missing:
    ...
    for item in components:
        ...
        if not registry_item:
            ...
            if registry_item.get("roadmapWaveId") != "wave_7_page_blocks":
                problems.append(f"registry-wave:{name}:{registry_item.get('roadmapWaveId')}")
            if registry_item.get("acceptanceContractId") != "ui-library-wave-7-page-blocks-v1":
    ...
    print("[check_ui_library_wave_7_page_blocks] OK components=%d" % len(components))
    return 0
