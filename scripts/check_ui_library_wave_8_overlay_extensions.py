...
ROOT = Path('.')
CONTRACT = ROOT / 'docs/02-architecture/context/ui-library-wave-8-overlay-extensions.v1.json'
...
REQUIRED_GATE_COMMAND = 'npm -C web run gate:ui-library-wave -- --wave wave_8_overlay_extensions'
EXPECTED_COMPONENTS = {'ContextMenu', 'TourCoachmarks'}


def main() -> int:
    missing = [path for path in (CONTRACT, API_CATALOG, REGISTRY) if not path.exists()]
    if missing:
        print('[check_ui_library_wave_8_overlay_extensions] FAIL')
        for path in missing:
    ...
    if data.get('wave_id') != 'wave_8_overlay_extensions':
        problems.append('invalid-wave-id')
    if data.get('family_id') != 'overlay':
    ...
    if seen != EXPECTED_COMPONENTS:
        problems.append(f'unexpected-components:{sorted(seen)}')
    ...
    for item in components:
        ...
        if not registry_item:
            ...
            if registry_item.get('roadmapWaveId') != 'wave_8_overlay_extensions':
                problems.append(f'registry-wave:{name}:{registry_item.get("roadmapWaveId")}')
            if registry_item.get('acceptanceContractId') != 'ui-library-wave-8-overlay-extensions-v1':
    ...
    print('[check_ui_library_wave_8_overlay_extensions] OK components=%d' % len(components))
    return 0
