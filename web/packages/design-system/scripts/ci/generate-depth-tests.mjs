...
/* ------------------------------------------------------------------ */
/*  Component discovery (shared pattern with contract generator)       */
/* ------------------------------------------------------------------ */
...
function parseProps(filePath) {
  ...
  return {
    ...
    functions,
    hasChildren,
    controlled: hasValue && hasOnChange,
    hasDefaultValue,
    allProps,
  };
}
...
function generateDepthTest(component, parsed) {
  ...
  // ── Section 1: Boolean prop combinations ──
  if (parsed.booleans.length >= 2) {
    const combos = parsed.booleans.slice(0, 4);
    const propsStr = combos.map(b => b).join(' ');
    sections.push(`  describe('${name} — depth: prop combinations', () => {
    it('renders with ${combos.join(' + ')} simultaneously', () => {
      render(<${name}${spread} ${propsStr}>Stressed</${name}>);
      expect(screen.queryByText('Stressed') || document.body.firstElementChild).toBeTruthy();
    });

    it('does not crash with all boolean props toggled', () => {
      const { container } = render(<${name}${spread} ${propsStr} />);
      expect(container.firstElementChild).toBeTruthy();
    });
  });`);
  } else if (parsed.booleans.length === 1) {
    const b = parsed.booleans[0];
    sections.push(`  describe('${name} — depth: prop combinations', () => {
    it('renders with ${b}', () => {
      const { container } = render(<${name}${spread} ${b} />);
      expect(container.firstElementChild).toBeTruthy();
    });
  });`);
  }

  // ── Section 2: Union prop matrix ──
  ...
  // ── Section 4: Array prop edge cases ──
  ...
  // ── Section 6: Controlled vs uncontrolled ──
...
` : ''}  });`);
  ...
  // ── Fallback: if no sections generated ──
...
${requiredPropsCode ? `${requiredPropsCode}\n` : ''}describe('${name} — depth', () => {
${sections.join('\n\n')}
});
`;
}
...
for (const dir of COMPONENT_DIRS) {
  ...
  for (const component of components) {
    ...
    if (fs.existsSync(testFile)) {
      // Check if existing test is real (has component import)
      const existing = fs.readFileSync(testFile, 'utf-8');
      const isReal = /import\s+.*from\s+['"]\.\.\//.test(existing);
      if (isReal) {
        totalExisting++;
        continue; // Don't overwrite real depth tests
      }
      // Fake test — will be overwritten
    }
